...
import Dustbin from "../React DnD/01 Dustbin/Copy or Move/Dustbin";
import Box from "../React DnD/01 Dustbin/Copy or Move/Box";
...
function GetHighchartSeries(objectOrUID) {
  ...
  let cols = rawCols.map((item, i, array) => {
    return {
      text: item["Name"],
      id: item["ColumnIndex"]
    };
      ...
      <div>
        ...
        {seriesCol.length > 0 && (
          <div>
            <div>
              <p>
                Parameters ({seriesCol.length} / {seriesCol.length})
              </p>
